...
| 问题 | 解决方法 |
|------|----------|
| 容器启动后立即退出 | `docker compose logs backend` 查看日志 |
| 数据库连接拒绝 | 检查 `depends_on` + `healthcheck` 配置 |
...
SECRET_KEY=${SECRET_KEY}          # 通过环境变量注入，不硬编码

# 数据库
# 使用非 root 用户、限制端口仅内部访问、定期备份

# HTTPS
# 生产环境必须启用，Nginx 配置 HTTP → HTTPS 重定向

# 输入校验
# 后端使用 Pydantic 校验 + 前端基础校验（双重保障）
```
